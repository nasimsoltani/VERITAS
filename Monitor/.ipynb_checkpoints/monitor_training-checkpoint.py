...
def train_model(model, train_dl, val_dl, channel_param, save_path):
	...
	epochs = 100           # Number of epochs you want to train for
	early_stopping = False  # Set to True or False to enable or disable early stopping
	...
	#model.load_state_dict(torch.load('results/weights.pt'))
	...
	for epoch in range(epochs):  # First for loop: loop over the dataset multiple times (epochs)
		...
		for batch_cntr, (anchor_input, positive_input, negative_input, labels) in enumerate(train_dl):   # Second for loop: in each epoch, loop over batches
			...
			_, anchor_outputs = model(anchor_input)
			_, positive_outputs = model(positive_input)
			_, negative_outputs = model(negative_input)
		...
		with torch.no_grad():     # We don't need gradients on to do reporting
			...
			for batch_cntr, vdata in enumerate(val_dl):
				...
				_, voutputs_anchor = model(vinputs_anchor)
				_, voutputs_positive = model(vinputs_positive)
				_, voutputs_negative = model(vinputs_negative)
			...
			if avg_vloss < best_vloss:
				...
				model_path = os.path.join(save_path,'weights_' + channel_param +'.pt')#'weights_'+str(epoch+1)+'.pt')
			...
			"""if epoch_patience_cntr == 3:    # halve the learning rate if validation loss does not improve for 3 epochs
			...
			if early_stopping and epoch_patience_cntr == patience:
				# stop training
